...
export default function InterviewerInterviewRoom() {
  ...
  useEffect(() => {
    fetch('/api/auth/me')
      .then((r) => r.json())
      .then((data) => {
        const name = data?.profile?.name || data?.user?.name || data?.user?.email?.split('@')[0] || 'Interviewer';
        setUserName(name);
      })
      .catch(() => {});
  }, []);
  ...
  const addSystemMessage = useCallback((text: string) => {
    setChatMessages((prev) => [...prev, {
      ...
      sender: 'system', senderName: 'System', text, timestamp: new Date(),
    }]);
  }, []);

  const pollRoom = useCallback(async () => {
    try {
      const res = await fetch(`/api/interview-room?sessionId=${sessionId}&role=interviewer`);
      if (!res.ok) return;
      const data = await res.json();

      // Only add messages from student side
      for (const msg of data.messages ?? []) {
        if (!processedMessages.current.has(msg.id)) {
          processedMessages.current.add(msg.id);
          if (msg.sender !== 'interviewer') {
            setChatMessages((prev) => [...prev, {
              id: msg.id, sender: msg.sender, senderName: msg.senderName,
              text: msg.text, timestamp: new Date(msg.timestamp),
            }]);
          }
        }
      }
      ...
      if (data.offer && !hasCreatedAnswer.current && pcRef.current.signalingState === 'stable') {
        ...
        await fetch('/api/interview-room', {
          ...
          body: JSON.stringify({
            sessionId, action: 'answer', role: 'interviewer',
    ...
    pc.onicecandidate = async (event) => {
      if (event.candidate) {
        await fetch('/api/interview-room', {
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            sessionId, action: 'ice-candidate', role: 'interviewer',
  ...
  // ✅ Fixed: show own message immediately
  const sendMessage = async () => {
    ...
    const res = await fetch('/api/interview-room', {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        sessionId, action: 'message', role: 'interviewer', senderName: userName, text,
      }),
    });
    ...
    setChatMessages((prev) => [...prev, {
      ...
      sender: 'interviewer', senderName: 'You', text, timestamp: new Date(),
    }]);
  };
  ...
  const statusColors = { waiting: 'bg-yellow-500', connecting: 'bg-blue-500', connected: 'bg-green-500', disconnected: 'bg-red-500' };
  const statusLabels = { waiting: 'Waiting for Student', connecting: 'Connecting…', connected: 'Live', disconnected: 'Disconnected' };

  return (
    <div className="flex h-screen bg-gray-950 text-white overflow-hidden" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div className="flex flex-col flex-1 min-w-0">
        ...
        <div className="flex items-center justify-between px-6 py-3 bg-gray-900 border-b border-gray-800">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => setIsNotesOpen((o) => !o)}
        ...
        <div className="flex-1 relative bg-gray-950 p-4">
          ...
          {/* Notes overlay */}
          {isNotesOpen && (
            <div className="absolute top-6 left-6 w-72 bg-gray-900/95 backdrop-blur-sm border border-gray-700 rounded-2xl shadow-2xl">
              ...
              <div className="p-3">
                <textarea
                  ...
                  className="w-full h-48 bg-gray-800 border border-gray-700 rounded-xl p-3 text-sm text-gray-200 placeholder-gray-600 focus:outline-none focus:border-amber-500/50 resize-none transition-colors"
                />
                <p className="text-xs text-gray-600 mt-2">Private — only visible to you</p>
              </div>
            </div>
          )}
        </div>
