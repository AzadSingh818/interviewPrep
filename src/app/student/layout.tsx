...
export default function StudentLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex overflow-x-hidden bg-gradient-to-br from-slate-50 via-white to-indigo-50 dark:from-[#09111f] dark:via-[#0b1324] dark:to-[#1a1b3a] transition-colors duration-200">
      ...
      ══════════════════════════════════════════════ */}
      <div className="fixed top-0 left-0 h-full w-12 sm:w-14 bg-white dark:bg-slate-950 border-r border-slate-200 dark:border-white/10 flex flex-col items-center py-3 z-50 shrink-0">
          ...
          {iconStripItems.map(item => {
            ...
            const btn = (
              <button
                ...
                className={`w-11 h-11 flex items-center justify-center rounded-lg transition-colors
                  ${isActive ? 'bg-slate-100 text-slate-900 dark:bg-indigo-500/15 dark:text-white' : 'text-slate-500 dark:text-slate-500 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-white/10'}`}
              >
                {item.icon}
              </button>
            );
            ...
            return (
              <Link key={item.href} href={item.href} title={item.title}>
                <div className={`w-11 h-11 flex items-center justify-center rounded-lg transition-colors
                  ${isActive ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-500/20 dark:text-indigo-300' : 'text-slate-500 dark:text-slate-500 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-white/10'}`}>
                  {item.icon}
                </div>
              </Link>
            );
        ...
        <div className="flex flex-col items-center gap-2 mt-auto">
          ...
          <button
            ...
            title="Logout"
            className="w-11 h-11 flex items-center justify-center rounded-lg text-slate-500 dark:text-slate-500 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-white/10 transition-colors"
          >
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-4 shrink-0">
          ...
          <button
            onClick={() => setSidebarOpen(false)}
            className="w-11 h-11 flex items-center justify-center rounded-lg text-slate-400 dark:text-slate-500 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-white/10 transition-colors touch-manipulation"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="flex-1 overflow-y-auto px-3 py-2 space-y-0.5">
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all text-[13.5px] font-medium group
                  ${isActive ? 'bg-indigo-50 text-indigo-700 dark:bg-indigo-500/15 dark:text-indigo-300' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-white/5 hover:text-slate-900 dark:hover:text-white'}`}
              >
                <span className={`shrink-0 transition-colors ${isActive ? 'text-indigo-600 dark:text-indigo-300' : 'text-slate-400 dark:text-slate-500 group-hover:text-slate-600 dark:group-hover:text-slate-200'}`}>
                  {item.icon}
                </span>
                {item.name}
              </Link>
          ...
          >
            <svg className="w-[18px] h-[18px] shrink-0 text-slate-400 dark:text-slate-500 group-hover:text-slate-600 dark:group-hover:text-slate-200 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {profile && (
            <div className="px-3 py-2">
              <p className="text-[11px] font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-2">Sessions Left</p>
          ...
          {/* Upgrade / Pro CTA */}
          {!isPro ? (
            <div className="mx-1 mt-1 p-3 bg-gradient-to-br from-indigo-50 to-violet-50 border border-indigo-100 rounded-xl dark:from-indigo-500/10 dark:to-violet-500/10 dark:border-indigo-400/20">
              <p className="text-[13px] font-semibold text-slate-800 dark:text-slate-100 mb-0.5">Go Pro 🚀</p>
            ...
            <div className="mx-1 mt-1 p-3 bg-indigo-50 border border-indigo-100 rounded-xl text-center dark:bg-indigo-500/10 dark:border-indigo-400/20">
              <p className="text-[13px] font-semibold text-indigo-600 dark:text-indigo-300">⭐ Pro Plan Active</p>
              <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">Enjoy unlimited access</p>
            </div>
          )}

          {/* Incomplete profile warning */}
          {!profileComplete && (
            <div className="mx-1 mt-2 p-3 bg-amber-50 border border-amber-200 rounded-xl dark:bg-amber-500/10 dark:border-amber-400/20">
              <p className="text-[11px] font-semibold text-amber-700 dark:text-amber-300 mb-1.5">⚠️ Complete your profile</p>
              <button
                onClick={() => { setSettingsOpen(true); setSidebarOpen(false); }}
        ...
        <div className="shrink-0 border-t border-slate-200 dark:border-white/10 px-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[11px] text-slate-500 dark:text-slate-400 truncate">{isPro ? '⭐ Pro Plan' : 'Free Plan'}</p>
            </div>
            <button
              onClick={handleLogout}
              title="Logout"
              className="shrink-0 w-7 h-7 flex items-center justify-center rounded-lg text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-white/10 transition-colors"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.8" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"/>
              </svg>
      ...
      <main className="flex-1 pl-12 sm:pl-14 min-w-0 w-full">
        {/* Top bar inside content area */}
        <div className="sticky top-0 z-30 bg-white/85 dark:bg-slate-950/80 backdrop-blur border-b border-slate-200 dark:border-white/10 px-3 sm:px-6 py-3 flex items-center justify-between gap-3">
