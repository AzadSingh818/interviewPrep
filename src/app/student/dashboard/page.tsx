      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6">
        ...
        <div className="shrink-0 text-center sm:text-left">
          <p className={`text-4xl sm:text-5xl font-bold ${scoreColor}`}>
            {score}<span className="text-lg sm:text-xl text-slate-400 font-normal">/100</span>
          </p>
  ...
  return (
    <Card variant="elevated" className="p-4 sm:p-6 mb-4 sm:mb-6">
      <div className="flex justify-between items-center mb-3 sm:mb-4">
        <h3 className="text-sm sm:text-base font-semibold text-slate-900">Recent Feedback</h3>
        <Link href="/student/sessions">
          <span className="text-xs sm:text-sm text-indigo-600 hover:underline cursor-pointer">View all →</span>
        </Link>
      </div>
      ...
      <div className="space-y-3 sm:space-y-4">
        {withFeedback.map(s => {
          ...
          return (
            <div key={s.id} className="p-3 sm:p-4 bg-slate-50 rounded-xl border border-slate-200">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-2 sm:mb-3">
                <div className="flex items-center gap-2">
                  <span className="text-sm sm:text-base">{isInterview ? '💼' : '🎓'}</span>
    ...
    return (
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center gap-3 mb-6 sm:mb-8">
          ...
          <h1 className="text-xl sm:text-2xl font-bold text-slate-900">Upgrade to Pro</h1>
        </div>
        <PaymentGate
    ...
    <div className="max-w-6xl mx-auto">
      ...
      <Card variant="elevated" className="p-4 sm:p-6 lg:p-8 mb-4 sm:mb-6">
        <div className="flex justify-between items-center mb-2">
          <h2 className="text-lg sm:text-2xl font-display font-semibold text-slate-900">Your Profile</h2>
        </div>
        ...
        {/* ── Resume Upload ── */}
        {profile && (
          <div id="resume" className="mt-6 sm:mt-8 pt-6 sm:pt-8 border-t border-slate-200">
            <h3 className="text-base sm:text-lg font-semibold text-slate-900 mb-3 sm:mb-4">Resume</h3>
            {profile.resumeUrl ? (
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 p-3 sm:p-4 bg-gradient-to-r from-green-50 to-emerald-50 border-2 border-green-200 rounded-xl">
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <p className="font-semibold text-slate-900 text-sm">Resume uploaded ✅</p>
                    <p className="text-xs sm:text-sm text-slate-600 truncate">{getResumeFileName(profile.resumeUrl)}</p>
                  </div>
                </div>
              ...
              <div className="border-2 border-dashed border-slate-300 rounded-xl p-6 sm:p-8 text-center hover:border-indigo-400 transition-colors">
                ...
                <h4 className="text-sm sm:text-base font-semibold text-slate-900 mb-1">Upload your resume</h4>
                <p className="text-xs sm:text-sm text-slate-500 mb-4">PDF, DOC, or DOCX · Max 5MB</p>
                ...
                {resumeError && <p className="text-xs sm:text-sm text-red-600 mt-2">{resumeError}</p>}
              </div>
            )}
          </div>
        )}
      ...
      <div className="grid grid-cols-3 gap-2 sm:gap-4 sm:gap-5 mb-4 sm:mb-6">
        <Card variant="bordered" className="p-3 sm:p-5">
          <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3">
            ...
            <div>
              <div className="text-xl sm:text-2xl font-bold text-indigo-600">{upcomingSessions.length}</div>
        ...
        <Card variant="bordered" className="p-3 sm:p-5">
          <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3">
            ...
            <div>
              <div className="text-xl sm:text-2xl font-bold text-green-600">{completedSessions.length}</div>
      ...
      <Card variant="elevated" className="p-4 sm:p-6 lg:p-8">
        <h2 className="text-lg sm:text-2xl font-display font-semibold text-slate-900 mb-4 sm:mb-5">Quick Actions</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
          <Link href="/student/book-guidance">
            <div className="p-4 sm:p-6 bg-gradient-to-br from-indigo-50 to-violet-50 border-2 border-indigo-200 rounded-xl hover:shadow-lg transition-all cursor-pointer group">
              ...
              <h3 className="font-semibold text-slate-900 mb-1 text-sm sm:text-base">Book Guidance Session</h3>
              <p className="text-xs sm:text-sm text-slate-600">Get mentorship from industry experts</p>
              {profile && (
          ...
          <Link href="/student/book-interview">
            <div className="p-4 sm:p-6 bg-gradient-to-br from-violet-50 to-pink-50 border-2 border-violet-200 rounded-xl hover:shadow-lg transition-all cursor-pointer group">
              <div className="text-2xl sm:text-3xl mb-2 sm:mb-3">💼</div>
              <h3 className="font-semibold text-slate-900 mb-1 text-sm sm:text-base">Book Mock Interview</h3>
              <p className="text-xs sm:text-sm text-slate-600">Practice with realistic interview scenarios</p>
              {profile && (
          ...
          <Link href="/student/sessions">
            <div className="p-4 sm:p-6 bg-gradient-to-br from-green-50 to-emerald-50 border-2 border-green-200 rounded-xl hover:shadow-lg transition-all cursor-pointer group">
              <div className="text-2xl sm:text-3xl mb-2 sm:mb-3">📅</div>
              <h3 className="font-semibold text-slate-900 mb-1 text-sm sm:text-base">My Sessions</h3>
              <p className="text-xs sm:text-sm text-slate-600">View your complete session history</p>
              {sessions.length > 0 && (
          ...
          >
            <div className="text-2xl sm:text-3xl mb-2 sm:mb-3">⭐</div>
            <h3 className="font-semibold text-slate-900 mb-1 text-sm sm:text-base">Upgrade to Pro</h3>
            <p className="text-xs sm:text-sm text-slate-600">Unlock more sessions &amp; priority mentors</p>
