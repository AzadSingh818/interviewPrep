...
const GUIDANCE_TOPICS = [
  ...
  'Offer Evaluation',
  'LinkedIn Profile',
  'College to Industry Transition',
  'Startup vs Big Tech',
  'Other',
];
...
// ─── Interviewer Avatar ───────────────────────────────────────────────────────
function InterviewerAvatar({ interviewer, size = 56 }: { interviewer: any; size?: number }) {
...
// ─── Booking Form — defined OUTSIDE main component so it never remounts ───────
function BookingForm({
  ...
  const handleOptionChange = (option: string) => {
    ...
    if (option !== 'Other') {
      ...
      // Keep topic empty until user types
  ...
  return (
    <Card variant="elevated" className="theme-surface-card p-4 sm:p-6 lg:sticky lg:top-8">
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        <TopicSelector
          selectedOption={topicOption}
          customText={customTopic}
        ...
        <Button type="submit" disabled={submitting || !selectedInterviewer} className="w-full">
                ...
                <div className="flex flex-wrap items-center gap-2 mt-2">
                  {interviewer.availabilitySlots?.length > 0 ? (
                    <span className="inline-flex items-center gap-1 text-xs font-medium text-green-700 bg-green-50 border border-green-200 px-2 py-0.5 rounded-full">
                      ...
                      {interviewer.availabilitySlots.length} slot
...
export default function BookGuidancePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setPlan(prev =>
          prev ? { ...prev, guidanceUsed: data.used, guidanceLimit: data.limit, planType: data.planType } : prev,
        );
      } else {
  ...
  return (
    <div className="max-w-6xl mx-auto px-0 sm:px-0">
      ...
      {/* ── Mobile: show form after mentor selected ── */}
        ...
        <>
          ...
          <div className="lg:hidden">
            ...
            <MentorList
              interviewers={interviewers}
              selectedInterviewer={selectedInterviewer}
              onSelect={(iv) => { setSelectedInterviewer(iv); setShowMobileForm(true); }}
              mobile={true}
            />
          </div>
        </>
